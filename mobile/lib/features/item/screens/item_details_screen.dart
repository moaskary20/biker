...
class _ItemDetailsScreenState extends State<ItemDetailsScreen> {
                    ...
                    return Container(
                      ...
                      child: CustomButton(
                        ...
                        onPressed: (!Get.find<SplashController>().configModel!.moduleConfig!.module!.stock! || stock! > 0) ?  () async {
                          // Make direct call to vendor
                          await _callVendor(itemController.item!);
                        } : null,
                      ),
                    );
                  }
  ...
  Future<void> _callVendor(Item item) async {
    try {
      if (item.storeId != null) {
        // Get store details to get phone number
        StoreController storeController = Get.find<StoreController>();
        Store? store = await storeController.getStoreDetails(Store(id: item.storeId), false);
        
        if (store != null && store.phone != null && store.phone!.isNotEmpty) {
          // Make direct call
          final Uri phoneUri = Uri(scheme: 'tel', path: store.phone!);
          if (await canLaunchUrl(phoneUri)) {
            await launchUrl(phoneUri);
          } else {
            showCustomSnackBar('cannot_make_call'.tr);
          }
        } else {
          showCustomSnackBar('vendor_phone_not_available'.tr);
        }
      } else {
        showCustomSnackBar('store_info_not_available'.tr);
      }
    } catch (e) {
