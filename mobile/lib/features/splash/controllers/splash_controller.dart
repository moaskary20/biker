...
class SplashController extends GetxController implements GetxService {
  ...
  Future<void> getConfigData({NotificationBodyModel? notificationBody, bool loadModuleData = false, bool loadLandingData = false, DataSourceEnum source = DataSourceEnum.local, bool fromMainFunction = false, bool fromDemoReset = false}) async {
    print('SplashController.getConfigData called');
    _hasConnection = true;
    _moduleIndex = 0;
    Response response;
    if(source == DataSourceEnum.local && !fromDemoReset) {
      print('Getting config from local');
      response = await splashServiceInterface.getConfigData(source: DataSourceEnum.local);
      print('Local config response: ' + response.statusCode.toString());
      ...
      print('Getting config from client');
      response = await splashServiceInterface.getConfigData(source: DataSourceEnum.client);
      print('Client config response: ' + response.statusCode.toString());
      _handleConfigResponse(response, loadModuleData, loadLandingData, fromMainFunction, fromDemoReset, notificationBody);
    }
  ...
  Future<void> _handleConfigResponse(Response response, bool loadModuleData, bool loadLandingData, bool fromMainFunction, bool fromDemoReset, NotificationBodyModel? notificationBody) async {
    print('SplashController._handleConfigResponse: ' + response.statusCode.toString());
    if(response.statusCode == 200) {
      ...
      print('Config load failed: ' + (response.statusText ?? 'no statusText'));
  ...
  _prepareLandingModel(LandingModel? landingModel) {
    if(landingModel != null) {
      ...
      // Initialize BlogController and load blog posts
      try {
        if(Get.isRegistered<BlogController>()) {
          print('SplashController: BlogController is registered, loading posts...');
          // Use Future.microtask to avoid setState during build
          Future.microtask(() {
            Get.find<BlogController>().getBlogPosts();
          });
        } else {
          print('SplashController: BlogController is NOT registered, trying to lazy load...');
          // Try to lazy load the BlogController
          Future.microtask(() {
            try {
              Get.find<BlogController>().getBlogPosts();
              print('SplashController: BlogController lazy loaded successfully');
            } catch (e) {
              print('SplashController: Error lazy loading BlogController: $e');
            }
          });
        }
      } catch (e) {
        print('SplashController: Error initializing BlogController: $e');
      }
    }
    update();
  }
